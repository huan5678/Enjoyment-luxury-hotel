...
//
import { ApiResponse, IRoom, RoomTypeCardProps } from '@/types';
...
export default function Page() {
  ...
  const [roomTypesList, setRoomTypeList] = useState<IRoom[]>([]);

  const getRoomType = async () => {
    await apiGetRoomType().then((res: ApiResponse<IRoom[] | IRoom | null>) => {
      if (res.status) {
        setRoomTypeList(res.result as IRoom[]);
      }
    });
  };
          ...
          <Stack py={matches ? 5 : '120px'}>
            ...
            <Box component="section" mt={matches ? 5 : 10}>
              <Grid container spacing={matches ? 3 : 6}>
                {roomTypesList.map((room: RoomTypeCardProps) => (
